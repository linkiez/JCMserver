...
export class PedidoComprasController {
    ...
    async init() {
        ...
        autocompleteFornecedor(this.inputFornecedor, await this.listaFornecedor.getFornecedores())
    }
}
...
function autocompleteFornecedor(inp: any, arr: any[]) {
        ...
        for (i = 0; i < arr.length; i++) {
          ...
          if (arr[i].nome.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
            ...
            /*execute a function when someone clicks on the item value (DIV element):*/
                ...
                let inputFornecedorId: HTMLInputElement= document.querySelector('#inputFornecedorId');
